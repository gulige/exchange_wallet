...
start_node() ->
    ?INFO("starting ssc node, cwd=~p~n", [file:get_cwd()]),
    Res = os:cmd("dir0=`pwd`; cd ./apps/exchange_sign/priv/ssc; ./start.sh; cd $dir0"),
    ?INFO("~p~n", [Res]),
    timer:sleep(5000),
    ok.

stop_node() ->
    os:cmd("./priv/ssc/stop.sh"),
    ok.
...
set_address_to_account(Addr, Account) ->
    lib_mongo:write_one(?CHAIN, <<"ssc_address_to_account">>, #{<<"_id">> => Addr, <<"account">> => Account}),
    ok.

get_account_from_address(Addr) ->
    #{<<"account">> := Account} = lib_mongo:find_one(?CHAIN, <<"ssc_address_to_account">>, #{<<"_id">> => Addr}, #{<<"account">> => true}),
    Account.

% 批量生成地址前，解锁1小时：chain_ssc:wallet_passphrase(3600).
get_new_address() ->
...
get_balance(?SYMBOL, Addr) ->
    ...
    BalanceL = [One / ?FACTOR || [_, #{<<"condition">> := #{<<"asset_id">> := 0}, <<"balance">> := One}] <- L],
    lists:sum(BalanceL);
get_balance(_Symbol, _Addr) ->
    0.

% SSC与多资产交易手续费内部固定是0.01SSC，合约交易大约为0.02SSC左右
make_tx(?SYMBOL, ToAddr, [{FromAddr, Amount}], Gas) ->
    FromAccount = get_account_from_address(FromAddr),
    {ok, Tx} = rpc_request(create_transfer_transaction, [float_to_binary(util:round10d(Amount - Gas), [{decimals, 5}, compact]), <<"SSC">>, FromAccount, ToAddr]),
