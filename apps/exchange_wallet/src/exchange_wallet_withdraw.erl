%%%------------------------------------
%%% @Module  : exchange_wallet_withdraw
%%% @Description: 钱包提币进程
%%%------------------------------------
-module(exchange_wallet_withdraw).
...
start_link(Chain) ->
    gen_server:start_link({local, binary_to_atom(<<"exchange_wallet_withdraw_worker_", Chain/binary>>, utf8)}, ?MODULE, [Chain], []).
...
handle_info(to_process, #status{chain = Chain} = Status) ->
    catch lib_withdraw:do_chain(Chain),
